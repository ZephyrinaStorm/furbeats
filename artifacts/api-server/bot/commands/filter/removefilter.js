...
const { FILTER_NAMES, applyFilters } = require("../../lib/filterPresets");
const { check_if_dj } = require("../../handlers/functions");

module.exports = {
  name: "removefilter",
  aliases: ["removefilters", "removef", "remf"],
  usage: "removefilter <name>",
  description: "Removes a specific filter from the active filter stack",
  category: "Filter",
  cooldown: 3,
  run: async (client, message, args) => {
    ...
    let activeFilters = client.infos.get(message.guild.id, "activeFilters") || [];
    if (!activeFilters.includes(name)) return message.reply({ embeds: [new EmbedBuilder().setColor(config.wrongcolor).setTitle(`🐾 Filter \`${name}\` is not active!`)] });
    ...
    message.reply({ embeds: [new EmbedBuilder().setColor(config.color).setTitle(`🎚️ Removed filter \`${name}\`!`).setDescription(activeFilters.length ? `Active: ${activeFilters.map(f => `\`${f}\``).join(", ")}` : "*No active filters*").setFooter({ text: config.footertext })] });
  },
};
