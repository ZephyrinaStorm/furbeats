...
module.exports = {
  ...
  usage: "addfilter <name>",
  description: "Adds a filter to the current filter stack",
  category: "Filter",
  cooldown: 3,
  run: async (client, message, args) => {
    ...
    if (activeFilters.includes(name)) return message.reply({ embeds: [new EmbedBuilder().setColor(config.wrongcolor).setTitle(`🐾 Filter \`${name}\` is already active!`)] });
    ...
    message.reply({ embeds: [new EmbedBuilder().setColor(config.color).setTitle(`🎚️ Added filter \`${name}\`!`).setDescription(`Active: ${activeFilters.map(f => `\`${f}\``).join(", ")}`).setFooter({ text: config.footertext })] });
  },
};
